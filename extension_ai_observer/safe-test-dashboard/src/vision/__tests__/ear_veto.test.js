// Ported from extension/test/ear_veto.test.js (READ-ONLY upstream) — faithful
// 1:1 conversion of the bespoke check/checkTrue harness to Vitest.
//
...
//      gate prevents.
//
// `makeReporter(gate)` below is exactly the contract the Phase 3 engine's
// reporting choke point (`_reportViolation`) must satisfy — kept faithful on
// purpose.

import { describe, it, expect } from 'vitest';
import '../testing/json_eq.js';
import * as veto from '../ear_veto.js';
import { syntheticFaceWithEar as landmarksWithEar } from '../testing/synthetic_face.js';
...
describe('ear_veto', () => {
  describe('THE REQUIRED ASSERTION: LOOKING_AWAY + EAR < 0.20 => ZERO events', () => {
  ...
  describe('eyes OPEN confirms the same violation', () => {
    const gate = new veto.EarVetoGate();
    const r = makeReporter(gate);
    for (let i = 0; i < 50; i++) {
      const t = 1000 + i * 100;
  ...
  describe('the 0.20 threshold, both sides', () => {
  ...
  describe('⚠ THE ALLOWLIST: what a blink may and may not silence', () => {
    const gate = new veto.EarVetoGate();
  ...
  describe('⚠ FAIL-OPEN when EAR is unavailable', () => {
  ...
  describe('⚠ a STALE reading cannot license suppression', () => {
    const gate = new veto.EarVetoGate();
  ...
  describe('one closed eye is enough to veto', () => {
    const gate = new veto.EarVetoGate();
  ...
  describe('THE CLASSIFIER AS A PRIMARY DETECTOR', () => {
  ...
  describe('⚠ hasFreshSample(): promotion must not outrun the safeguard', () => {
  ...
  describe('eyesVerifiablyClosed() (used to withhold classifier escalation)', () => {
    const gate = new veto.EarVetoGate();
  ...
  describe('a blink mid-violation vetoes only the blinking frames', () => {
  ...
  describe('reset clears the gate', () => {
    const gate = new veto.EarVetoGate();
    gate.submitLandmarks(landmarksWithEar(0.05), 1000);
