// Ported from extension/test/gaze_landmarks.test.js (READ-ONLY upstream) —
// faithful 1:1 conversion of the bespoke `check(name, actual, expected)` /
// `checkTrue(name, cond)` harness to Vitest's describe/it/expect. Every
// assertion, its name, its expected value and its execution order are
// preserved; only the harness changed.
//
...
import * as g from '../gaze_landmarks.js';
import { syntheticEye as eye, syntheticFace as face, HEADS, scriptedFrames } from '../testing/synthetic_face.js';

const C = g.LANDMARK_CONTRACT;

/** Mirrors the upstream `check`/`checkTrue` exactly: JSON.stringify equality. */
...
/** Upstream's `run()`, rebuilt on the shared `scriptedFrames` generator. */
function run(analyzer, makeLandmarks, head, frames, dtMs = 100, t0 = 1000) {
  const events = [];
  let last = null;
  for (const frame of scriptedFrames({ builder: (i) => makeLandmarks(i), count: frames, dtMs, t0 })) {
    last = analyzer.process(frame.value, head, frame.t);
    events.push(...last.events);
  }
  return { events, last };
}

describe('gaze_landmarks', () => {
  describe('STEP 1: the EAR blink gate', () => {
  ...
  describe('STEP 2: horizontal iris ratio, and the inner/outer sign trap', () => {
  ...
  describe('STEP 3: vertical iris ratio (downward peeking)', () => {
  ...
  describe('geometry is illumination- and pigmentation-free by construction', () => {
  ...
  const TURNED_HEAD = HEADS.TURNED;
  /** Inside the calibrated band, but past the absolute 15-degree bound. */
  const SKEWED_HEAD = HEADS.SKEWED;
  /** Calibrated, neutral excursion, but the pose pipeline could not measure yaw. */
  const UNMEASURED_YAW_HEAD = HEADS.UNMEASURED_YAW;

  describe('30 SECONDS OF CLOSED EYES EMITS ZERO ALERTS', () => {
  ...
  describe('30 s of closed eyes while looking hard off-axis', () => {
  ...
  describe('blinking during honest work never accumulates', () => {
    const a = new g.GazeLandmarkAnalyzer();
  ...
  describe('calibration, then a forward-looking student', () => {
    const a = new g.GazeLandmarkAnalyzer();
  ...
  describe('STEP 4: sustained side gaze IS reported', () => {
    const a = new g.GazeLandmarkAnalyzer();
  ...
  describe('a brief glance is NOT reported', () => {
    const a = new g.GazeLandmarkAnalyzer();
    run(a, () => face({ h: 0.5 }), NEUTRAL_HEAD, 80);
  ...
  describe('gaze stands down when the head has turned', () => {
    const a = new g.GazeLandmarkAnalyzer();
    run(a, () => face({ h: 0.5 }), NEUTRAL_HEAD, 80);
  ...
  describe('reporting is suppressed during a liveness challenge', () => {
    const a = new g.GazeLandmarkAnalyzer();
    run(a, () => face({ h: 0.5 }), NEUTRAL_HEAD, 80);
    const events = [];
    for (let i = 0; i < 60; i++) {
  ...
  describe('missing landmarks are UNKNOWN, never a finding', () => {
    const a = new g.GazeLandmarkAnalyzer();
  ...
  describe('absolute head-yaw bound (spec: abs(Head_Yaw) < 15)', () => {
