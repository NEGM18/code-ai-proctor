...
// factory-argument destructuring that followed it (lines 23-41). All three are
// replaced by the static ESM imports below — the directive existed only for
// the extension browser-globals loading model, which these imports supersede.
import { computeHeadPose, ratioToApproxDegrees } from './pose_geometry.js';
import { PoseBaseline } from './pose_calibration.js';
import { TemporalSmoother, DwellGate, GateState } from './temporal_gate.js';

// ⚠ The factory body below (upstream lines 42-271) keeps its original 2-space
...
// >>> ESM PORT
// Upstream closed the factory with `return { … };` + `}));` (lines 272-273).
