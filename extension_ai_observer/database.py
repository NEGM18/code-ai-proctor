...
class CheatingIncident(Base):
    ...
    proctor_state_id: Mapped[Optional[int]] = mapped_column(ForeignKey("student_proctor_states.id"), nullable=True, index=True)
    cheat_reason: Mapped[Optional[str]] = mapped_column(String(255), nullable=True)
...
class StudentProctorState(Base):
    __tablename__ = "student_proctor_states"

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    session_id: Mapped[int] = mapped_column(ForeignKey("proctor_sessions.id"), nullable=False, index=True)
    student_id: Mapped[int] = mapped_column(ForeignKey("students.id"), nullable=False, index=True)
    status: Mapped[str] = mapped_column(String(20), nullable=False, default="not_started")
    started_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True), nullable=True)
    completed_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True), nullable=True)
    last_seen_heartbeat: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True), nullable=True)
    timing_status: Mapped[Optional[str]] = mapped_column(String(50), nullable=True, default="ON_TIME_BEFORE_QUIZ")
    quiz_opened_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True), nullable=True)
...
def _ensure_incident_schema() -> None:
    """Small compatibility migration for existing databases to add appeal_text, proctor_state_id, and cheat_reason."""
    if _engine is None:
        return
    insp = inspect(_engine)
    ...
    if "proctor_state_id" not in cols:
        dialect = _engine.dialect.name
        with _engine.begin() as conn:
            if dialect == "sqlite":
                conn.execute(text("ALTER TABLE cheating_incidents ADD COLUMN proctor_state_id INTEGER NULL"))
            elif dialect == "mysql":
                conn.execute(text("ALTER TABLE cheating_incidents ADD COLUMN proctor_state_id INT NULL"))
            else:
                conn.execute(text("ALTER TABLE cheating_incidents ADD proctor_state_id INT NULL"))
    if "cheat_reason" not in cols:
        dialect = _engine.dialect.name
        with _engine.begin() as conn:
            if dialect == "sqlite":
                conn.execute(text("ALTER TABLE cheating_incidents ADD COLUMN cheat_reason TEXT NULL"))
            elif dialect == "mysql":
                conn.execute(text("ALTER TABLE cheating_incidents ADD COLUMN cheat_reason VARCHAR(255) NULL"))
            else:
                conn.execute(text("ALTER TABLE cheating_incidents ADD cheat_reason VARCHAR(255) NULL"))


def _ensure_proctor_state_schema() -> None:
    """Ensure timing_status and quiz_opened_at exist on student_proctor_states table."""
    if _engine is None:
        return
    insp = inspect(_engine)
    if "student_proctor_states" not in insp.get_table_names():
        return
    cols = {c["name"] for c in insp.get_columns("student_proctor_states")}
    dialect = _engine.dialect.name
    with _engine.begin() as conn:
        if "timing_status" not in cols:
            if dialect == "sqlite":
                conn.execute(text("ALTER TABLE student_proctor_states ADD COLUMN timing_status VARCHAR(50) DEFAULT 'ON_TIME_BEFORE_QUIZ'"))
            elif dialect == "mysql":
                conn.execute(text("ALTER TABLE student_proctor_states ADD COLUMN timing_status VARCHAR(50) DEFAULT 'ON_TIME_BEFORE_QUIZ'"))
            else:
                conn.execute(text("ALTER TABLE student_proctor_states ADD timing_status VARCHAR(50) DEFAULT 'ON_TIME_BEFORE_QUIZ'"))
        if "quiz_opened_at" not in cols:
            if dialect == "sqlite":
                conn.execute(text("ALTER TABLE student_proctor_states ADD COLUMN quiz_opened_at DATETIME NULL"))
            elif dialect == "mysql":
                conn.execute(text("ALTER TABLE student_proctor_states ADD COLUMN quiz_opened_at DATETIME NULL"))
            else:
                conn.execute(text("ALTER TABLE student_proctor_states ADD quiz_opened_at DATETIME NULL"))
        if "end_reason" not in cols:
            if dialect == "sqlite":
                conn.execute(text("ALTER TABLE student_proctor_states ADD COLUMN end_reason VARCHAR(50) NULL"))
            elif dialect == "mysql":
                conn.execute(text("ALTER TABLE student_proctor_states ADD COLUMN end_reason VARCHAR(50) NULL"))
            else:
                conn.execute(text("ALTER TABLE student_proctor_states ADD end_reason VARCHAR(50) NULL"))
...
# ---------------------------------------------------------------------------
# Proctor Session & State helpers
# ---------------------------------------------------------------------------

def create_proctor_session(db: Session, teacher_id: int, name: str) -> ProctorSession:
    # Generate a unique 6-character alphanumeric code
    while True:
        code = "".join(random.choices(string.ascii_uppercase + string.digits, k=6))
        # Verify uniqueness
        exists = db.scalar(select(ProctorSession).where(ProctorSession.session_code == code))
        if not exists:
            break
...
def get_proctor_session_by_code(db: Session, code: str) -> ProctorSession | None:
    if not code:
        return db.scalars(select(ProctorSession).order_by(ProctorSession.id.desc())).first()
    clean_code = code.upper().strip()
    session = db.scalar(select(ProctorSession).where(ProctorSession.session_code == clean_code))
    if session:
        return session
    # Fallback to the latest active session if code is AUTO/TEST/unmatched
    return db.scalars(select(ProctorSession).order_by(ProctorSession.id.desc())).first()


def join_proctor_session(db: Session, session_code: str, student_id_str: str, full_name: str, timing_status: str | None = None, quiz_opened_at: str | None = None) -> StudentProctorState:
    session = get_proctor_session_by_code(db, session_code)
    if not session:
        raise ValueError(f"No active proctoring session found in database.")
        
    # Find student by student_id, email, or full name
    student = db.scalar(select(Student).where(Student.student_id == student_id_str.strip()))
    if not student:
        student = db.scalar(select(Student).where(Student.email == student_id_str.strip()))
    if not student:
        student = db.scalar(select(Student).where(Student.full_name == full_name.strip()))
    if not student:
        # Create student on the fly if taking quiz without prior DB entry
        raw_id = student_id_str.strip() or "student"
        email = raw_id.lower() if "@" in raw_id else f"{raw_id.lower()}@gmail.com"
        teacher = db.get(Teacher, session.teacher_id)
        org_id = teacher.organization_id if teacher else 1
        student = Student(
            organization_id=org_id,
            email=email,
            student_id=raw_id,
            full_name=full_name.strip() or "Student User",
    ...
    # Check if student proctor state already exists
    state = db.scalar(
        select(StudentProctorState).where(
            StudentProctorState.session_id == session.id,
            StudentProctorState.student_id == student.id
        )
    )
    
    now = datetime.now(timezone.utc)
    parsed_quiz_opened = None
    if quiz_opened_at:
        try:
            parsed_quiz_opened = datetime.fromisoformat(quiz_opened_at.replace("Z", "+00:00"))
        except Exception:
            pass

    if not state:
        state = StudentProctorState(
            session_id=session.id,
            student_id=student.id,
            status="active",
            started_at=now,
            last_seen_heartbeat=now,
            timing_status=timing_status or "ON_TIME_BEFORE_QUIZ",
            quiz_opened_at=parsed_quiz_opened
        )
        db.add(state)
    else:
        state.status = "active"
        ...
        state.last_seen_heartbeat = now
        if timing_status:
            state.timing_status = timing_status
        if parsed_quiz_opened:
            state.quiz_opened_at = parsed_quiz_opened
...
def update_proctor_heartbeat(db: Session, session_code: str, student_id_str: str) -> None:
    session = get_proctor_session_by_code(db, session_code)
    if not session:
        return
    ...
    if state:
        state.last_seen_heartbeat = datetime.now(timezone.utc)
        db.commit()


def log_proctor_incident(db: Session, session_code: str, student_id_str: str, 
                         cheat_probability: float, cheat_reason: str, 
                         snapshot_b64: str | None = None) -> CheatingIncident | None:
    session = get_proctor_session_by_code(db, session_code)
    if not session:
        return None
    ...
    if not student:
        student = db.scalar(select(Student).where(Student.email == student_id_str.strip()))
    if not student:
        student = db.scalar(select(Student).where(Student.full_name == student_id_str.strip()))
    ...
    if student:
        state = db.scalars(
            select(StudentProctorState).where(
                StudentProctorState.session_id == session.id,
                StudentProctorState.student_id == student.id
            ).order_by(StudentProctorState.id.desc())
        ).first()
    else:
        # Fallback to the latest active student proctor state in this session
        state = db.scalars(
            select(StudentProctorState).where(StudentProctorState.session_id == session.id)
            .order_by(StudentProctorState.id.desc())
        ).first()
        if state:
    ...
    incident = CheatingIncident(
        ...
        detected_at=datetime.now(timezone.utc),
        acknowledged=False,
        proctor_state_id=state.id if state else None,
...
def get_proctor_session_roster(db: Session, session_id: int) -> list[dict]:
    ...
    # Get all students who joined this session OR belong to the teacher's org
    session_student_ids = db.scalars(
        select(StudentProctorState.student_id).where(StudentProctorState.session_id == session_id)
    ).all()
    
    query = select(Student)
    if session_student_ids:
        query = query.where((Student.id.in_(session_student_ids)) | (Student.organization_id == org_id))
    else:
        query = query.where(Student.organization_id == org_id)
    ...
    for s in students:
        state = db.scalars(
            select(StudentProctorState).where(
                StudentProctorState.session_id == session_id,
                StudentProctorState.student_id == s.id
            ).order_by(StudentProctorState.id.desc())
        ).first()
        ...
        incidents = []
        if state:
            incidents_count = db.scalar(
                select(func.count(CheatingIncident.id)).where(
                    CheatingIncident.proctor_state_id == state.id
                )
            ) or 0
            
            incidents_rows = db.execute(
                select(CheatingIncident).where(CheatingIncident.proctor_state_id == state.id)
                .order_by(CheatingIncident.detected_at.asc())
            ).scalars().all()
            
            for inc in incidents_rows:
                incidents.append({
                    "id": inc.id,
                    "cheat_reason": inc.cheat_reason or "Unknown",
                    "cheat_probability": inc.cheat_probability,
                    "snapshot_b64": inc.snapshot_b64 or "",
        ...
        started_at = ""
        last_seen = ""
        timing_status = "ON_TIME_BEFORE_QUIZ"
        quiz_opened_at = ""
        
        if state:
            status = state.status
            started_at = str(state.started_at) if state.started_at else ""
            last_seen = str(state.last_seen_heartbeat) if state.last_seen_heartbeat else ""
            timing_status = state.timing_status or "ON_TIME_BEFORE_QUIZ"
            quiz_opened_at = str(state.quiz_opened_at) if state.quiz_opened_at else ""
            
            # Check if active but heartbeat is too old (e.g. lost connection/closed tab)
            # 30 seconds threshold
            if status == "active" and state.last_seen_heartbeat:
                diff = (datetime.now(timezone.utc) - state.last_seen_heartbeat.replace(tzinfo=timezone.utc)).total_seconds()
                if diff > 30:
                    status = "disconnected"
        ...
        result.append({
            ...
            "last_seen_heartbeat": last_seen,
            "timing_status": timing_status,
            "quiz_opened_at": quiz_opened_at,
            "end_reason": state.end_reason if state and hasattr(state, 'end_reason') and state.end_reason else "N/A",
            "ended_at": str(state.completed_at) if state and state.completed_at else "N/A",
...
def get_teacher_proctor_sessions(db: Session, teacher_id: int) -> list[ProctorSession]:
    return db.execute(
        select(ProctorSession).where(ProctorSession.teacher_id == teacher_id)
        .order_by(ProctorSession.created_at.desc())
    ).scalars().all()