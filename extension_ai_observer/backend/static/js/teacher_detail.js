...
document.querySelectorAll("#mainNav button[data-main-tab]").forEach(b=>{b.addEventListener("click",()=>{
  ...
  $("proctoringPanel").style.display=t==="proctoring"?"block":"none";
  if (t === "proctoring") {
    loadProctorSessions();
  }
})});
...
window._ackNotifDetail=async function(id,btn){
  ...
  async function loadProctorSessions() {
    ...
    if (!c.email) return;
    try {
      const dash = await jf("/api/teacher/dashboard", {
        method: "POST",
        body: JSON.stringify(c)
      });
      activeTeacherId = dash.teacher?.id;
      if (!activeTeacherId) return;

      const res = await fetch(`/api/proctor/sessions?teacher_id=${activeTeacherId}`);
      const data = await res.json();
      proctorSessions = data.sessions || [];
      renderProctorSessions();
    } catch (err) {
      console.error("Failed to load proctor sessions:", err);
    }
  }

  function renderProctorSessions() {
    const cont = $("proctorSessionsContainer");
    if (!cont) return;
    if (!proctorSessions.length) {
      cont.innerHTML = '<div class="empty-state"><p>No proctoring sessions yet. Create one above!</p></div>';
      return;
    }
    cont.innerHTML = proctorSessions.map(s => `
      <div class="class-card proctor-session-card" data-sid="${s.id}" style="padding: 1.25rem;">
        <h3 style="margin:0 0 0.5rem;color:#fff">${s.name}</h3>
        <div class="meta" style="font-size: 0.8rem;color:var(--text-secondary)">Code: <strong style="color:var(--accent);font-size:1rem">${s.session_code}</strong></div>
        <div class="meta" style="font-size: 0.75rem;color:var(--text-secondary);margin-top:0.5rem">Created: ${new Date(s.created_at).toLocaleDateString()}</div>
      </div>
    `).join("");

    cont.querySelectorAll(".proctor-session-card").forEach(card => {
      card.addEventListener("click", () => {
        const sid = parseInt(card.dataset.sid);
        const s = proctorSessions.find(x => x.id === sid);
        if (s) {
          openProctorSessionDetail(s);
        }
      });
    });
  }

  $("createProcSessionBtn")?.addEventListener("click", async () => {
    const name = $("proc_session_name").value.trim();
    if (!name) {
      msg("procSessionCreateMsg", "Please enter a session name", "error");
      return;
    }
    msg("procSessionCreateMsg", "Creating...", "");
    try {
      const res = await jf("/api/proctor/sessions/create", {
        method: "POST",
        body: JSON.stringify({
          session_name: name,
      ...
      if (res.ok) {
        msg("procSessionCreateMsg", "Session created successfully!", "ok");
        $("proc_session_name").value = "";
        loadProctorSessions();
      }
    } catch (err) {
      msg("procSessionCreateMsg", err.message, "error");
    }
  });

  async function openProctorSessionDetail(session) {
    selectedSession = session;
    $("sessionRosterCard").style.display = "block";
    $("rosterSessionName").textContent = session.name;
    $("rosterSessionCode").textContent = session.session_code;
    
    await loadSessionRoster();
  }

  async function loadSessionRoster() {
    if (!selectedSession) return;
    try {
      const res = await fetch(`/api/proctor/sessions/${selectedSession.id}/roster`);
      const data = await res.json();
  ...
  function renderSessionRoster() {
    const tbody = $("proctorRosterBody");
    if (!tbody) return;
    if (!currentRoster.length) {
      tbody.innerHTML = '<tr><td colspan="7" style="text-align:center">No students registered in organization roster</td></tr>';
      return;
    }

    tbody.innerHTML = currentRoster.map(s => {
      let statusBadgeClass = "tag";
      let statusColor = "var(--text-secondary)";
      let statusBg = "rgba(255,255,255,0.05)";
      
      if (s.status === "active") {
        statusBadgeClass = "tag active";
      ...
      const hasCheated = s.incidents_count > 0;
      const cheatColor = hasCheated ? "var(--error)" : "var(--success)";

      return `
        <tr>
          <td>${s.student_id_str}</td>
          <td><strong>${s.full_name}</strong></td>
          <td>${s.email}</td>
          <td>
            <span style="padding:0.25rem 0.5rem;border-radius:4px;font-size:0.75rem;font-weight:bold;background:${statusBg};color:${statusColor}">
              ${s.status.toUpperCase()}
            </span>
          </td>
          <td>${s.started_at ? new Date(s.started_at).toLocaleTimeString() : '—'}</td>
          <td style="color:${cheatColor};font-weight:bold">
            ${s.incidents_count}
          </td>
          <td>
            ${hasCheated ? `
              <button class="ghost" style="font-size:0.75rem;padding:0.25rem 0.5rem" onclick="window._reviewStudentIncidents('${s.student_id_str}', '${s.full_name.replace(/'/g, "\\'")}')">
                Review Snapshots
              </button>
            ` : '—'}
          </td>
  ...
  $("refreshRosterBtn")?.addEventListener("click", loadSessionRoster);
  
  $("exportRosterBtn")?.addEventListener("click", () => {
    if (!selectedSession) return;
    window.location.href = `/api/proctor/sessions/${selectedSession.id}/export`;
  });

  window._reviewStudentIncidents = function(studentIdStr, name) {
    const student = currentRoster.find(s => s.student_id_str === studentIdStr);
    if (!student || !student.incidents || !student.incidents.length) return;
    
    let html = `
      <div style="padding:1.5rem;color:white;max-height:80vh;overflow-y:auto;width:100%">
        <h3 style="margin-bottom:1rem;font-size:1.4rem">Incidents for ${name}</h3>
        <p class="sub" style="margin-bottom:1.5rem;color:var(--text-secondary)">Total Flags: <strong style="color:var(--error)">${student.incidents_count}</strong></p>
        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:1rem">
    `;
    
    student.incidents.forEach(inc => {
      const src = inc.snapshot_b64.startsWith('data:') ? inc.snapshot_b64 : 'data:image/jpeg;base64,' + inc.snapshot_b64;
      html += `
        <div style="background:var(--surface-2, #2d2d3d);border:1px solid var(--border);border-radius:8px;overflow:hidden;padding:0.75rem;display:flex;flex-direction:column;gap:0.5rem">
          <img src="${src}" style="width:100%;height:140px;object-fit:cover;cursor:pointer;border-radius:6px;border:1px solid rgba(255,255,255,0.05)" onclick="window._openSnapshotModal('${src}', '')" />
          <div style="font-size:0.8rem;margin-top:0.25rem;display:flex;flex-direction:column;gap:2px">
            <strong style="color:#fff">${inc.cheat_reason}</strong>
            <span style="color:var(--text-secondary)">${new Date(inc.detected_at).toLocaleTimeString()}</span>
            <span style="color:var(--accent);font-weight:bold">Prob: ${(inc.cheat_probability * 100).toFixed(1)}%</span>
          </div>
        </div>
      `;
    });
    ...
    Object.assign(box.style, {
      backgroundColor: 'var(--bg-card, #1c1c24)',
